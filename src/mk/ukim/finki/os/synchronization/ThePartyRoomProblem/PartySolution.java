...
import mk.finki.os.project.ProblemExecution;
import mk.finki.os.project.TemplateThread;
...
public class PartySolution {
	...
	static Semaphore mutex = new Semaphore(1); // sinhronizacija na if razgranuvanjeto
	static Semaphore turn = new Semaphore(1); // koj e na red da proba da vleze
	static Semaphore clear = new Semaphore(0); //signalizira na dekanot koga sobata e prazna
	static Semaphore lieIn = new Semaphore(0); //signalizira na dekanot da vleze ako cekal pred sobata
	...
	static class Student extends TemplateThread {
	    ...
	    state.dance();
			mutex.acquire();
				...
				//ako dekanot e vnatre, posledniot student mu signalizira deka sobata e prazna
		...
		public void execute() throws InterruptedException {
			...
			//vo ovoj uslov dekanot ceka na studentite
				...
				//dekanot nema potreba da ceka na studentite
					...
					//ako brojot na studenti e 0 dekanot vleguva i go izvrsuva prebaruvanjeto
					state.deanEnter();
					state.conductSearch();
