...
public class DiningPhilosophers {
	...
	//comments of the form /*** <comment text here> ***/ point out places the actual solution is implemented
	...
	public static class Philosopher extends TemplateThread {
		...
		public int getFirstFork(){
			...
			return Math.min(this.index, (this.index + 1) % DiningPhilosophers.NUMBER_OF_PHILOSOPHERS);
		}
		
		public int getSecondFork(){
			/*** Place to choose second fork ***/
			return Math.max(this.index, (this.index + 1) % DiningPhilosophers.NUMBER_OF_PHILOSOPHERS);
		}
	...
	public static void main(String[] args) {
		//removed [for i = 1 to 10 do run()] loop here - there are settings in run() to mimic similar behavior
		run();
	}

	public static void run() {
		try {
			DiningPhilosophers.NUMBER_OF_PHILOSOPHERS = 5; //default initial value
			int numRuns = 5;	//number of times will Philosopher.execute() be executed
			int numIterations = 1;	//default number of iterations tested
			int step = 5;	//the increment for NUMBER_OF_PHILOSOPHERS on each iteration
			
			/* may become useful for building test cases
			Scanner s = new Scanner(System.in);
			DiningPhilosophers.NUMBER_OF_PHILOSOPHERS = s.nextInt();
			numRuns = s.nextInt();
			...
			*/
			...
			for(int a = 0; a < numIterations; a++){
				System.out.println("\nTesting " + DiningPhilosophers.NUMBER_OF_PHILOSOPHERS + " philosophers.");
