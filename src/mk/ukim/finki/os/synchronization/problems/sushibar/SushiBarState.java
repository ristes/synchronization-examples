...
public class SushiBarState extends AbstractState {
	...
	public void seat(){
		if (groupOnTable){
			log(new PointsException(
					GROUP_IN_THE_BAR_POINTS,
					GROUP_IN_THE_BAR), null);
		}
	...
	public void eat(){
		...
		if ((peopleEating.getValue() == 5) && (!groupOnTable))
			log(new PointsException(GROUP_NOT_GATHERED_POINTS, GROUP_NOT_GATHERED), null);
		Switcher.forceSwitch(10);
	}
	
	/*
	 * Petiot posetitel mu signalizira na barot deka clenovite na masata se grupa
	 */
	public void groupGathered(){
		...
		log(customersAtTable.assertEquals(5, NOT_ENOUGH_CUSTOMERS_POINTS, NOT_ENOUGH_CUSTOMERS), null);
		groupOnTable = true;
	}
	
	/*
	 * Posledniot clen na grupata mu signalizira na barot deka se zavrseni i deka masata e slobodna
	 */
	public void groupDone(){
		...
		if (!groupOnTable){
			log(new PointsException(
					5,
					"Nemalo grupa vo barot za da si odi!!!"), null);
		}
		groupOnTable = false;
		// Namaluvanje na brojot vo barot
				log(customersAtTable.decrementWithMin(false), "Posledniot clen na grupata zaminuva");
				peopleEating.decrementWithMin(false);

				// Proverka dali posetitelot e posledniot clen na grupata
				log(customersAtTable.assertEquals(0, NOT_LAST_MEMBER_OF_GROUP_POINTS, NOT_LAST_MEMBER_OF_GROUP), null);
				if (!checkForParallelismAfterGroup){
	...
	public void done(){
		...
		log(customersAtTable.assertNotEquals(0, 5, "Nema clenovi koi bi mozele da zaminat!!!"), null);
	...
	public void finalize() {
		...
		if (customersAtTable.getMax() == 1) {
			logException(new PointsException(10,
					"ne e paralelno Sednuvanjeto"));
		}
		// Proverka dali e dozoleno da vlezat 5 clena vo barot
		if (customersAtTable.getMax() != 5) {
			logException(new PointsException(10,
					"Ne e sostavena grupa"));
		}
		// proverka dali jadenjeto e paralelno
		if (peopleEating.getMax() == 1) {
			logException(new PointsException(10,
					"ne e paralelno jadenjeto"));
		}
		// Proverka dali sednuvanjeto ostanuva paralelno po zaminuvanje na grupata
		if (maxAfterGroup == 1){
			logException(new PointsException(10,
