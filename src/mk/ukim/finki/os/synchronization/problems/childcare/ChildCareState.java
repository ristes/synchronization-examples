...
public class ChildCareState extends AbstractState {
	...
	private static final String CHILDREN_ENTERING_DEADLOCK = "Children entering deadlock... Children should be able to enter when numberOfChildren <= 3 * (numberOfAdults - 1)";
	...
	public void childrenEntered(int n) {
		synchronized (this) {
			// System.out.println(n == 1 ? "Child entered!" : (n +
			// " children entered!"));
			log(null, (n == 1 ? "Child entered!" : (n + " children entered!")));
			if (childrenEntering > 0)
				childrenEntering -= Math.max(n, childrenEntering);
			children += n;
		}
	}
